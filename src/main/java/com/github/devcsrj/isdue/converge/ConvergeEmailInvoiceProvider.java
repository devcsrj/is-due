 ...
 *
 * <p>Converge is an ISP in the Philippines - https://www.convergeict.com/. They send invoices via
 * email using {@code bssnotification@convergeict.com}.
 */
class ConvergeEmailInvoiceProvider implements InvoiceProvider {
  ...
  private static final Pattern TOTAL_AMOUNT_PATTERN =
      Pattern.compile("Your total amount due is: P (\\d+\\.\\d+)");
  ...
  private Optional<Invoice> readInvoice(ReceivedEmail email) {
    ...
    for (var message : messages) {
      ...
      var issuedDate =
          ZonedDateTime.ofInstant(email.receivedDate().toInstant(), ZoneId.of("Asia/Manila"));
      var dueDate = this.parseDueDate(content);
      var totalAmount = this.parseTotalAmount(content);

      var invoice =
          new Invoice(
              id, Converge.BILLER, accountNumber, issuedDate, dueDate, totalAmount, List.of());
  ...
  static class Filters {
    ...
    static EmailFilter sender() {
      return filter().from("bssnotification@convergeict.com").and().subject("Payment Reminder");
    }
